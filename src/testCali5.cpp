...
int main()
{
	...
	for (int i = 0; i < CHECKERBOARD[1]; i++)		// CHECKERBOARD[0] = 6
	{
		...
		{
			objp.push_back(cv::Point3f(j * 10, i * 10, 0));	// z는 0 이고 (x, y ,z)로 담기니까 (j, i , 0)으로 벡터에 push_back으로 값 담기
	...
	for (int i = 0; i < images.size(); i++)				// 받아온 이미지들은 images에 경로가? 저장되어있음. 한장씩 불러온다
										// 매번 이미지에 대해 CHESSBOARD를 그리고 창 띄우기
	{
...
/*	
	cv::Mat rmap[2][2];
	initUndistortRectifyMap(cameraMatrix_left, distCoeffs_left, R1, P1, imgsize, CV_16SC2, rmap[0][0], rmap[0][1]);
	initUndistortRectifyMap(cameraMatrix_right, distCoeffs_right, R2, P2, imgsize, CV_16SC2, rmap[1][0], rmap[1][1]);
	
	cv::Mat ldst, rdst;
	remap(lrImg[0], ldst, rmap[0][0], rmap[0][1], INTER_LINEAR);
	remap(lrImg[1], rdst, rmap[1][0], rmap[1][1], INTER_LINEAR);
	imshow("ldst",ldst);
	imshow("rdst",rdst);
