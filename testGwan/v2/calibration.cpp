...
int main()
{
	...
	*/
	...
	cv::FileStorage fs("90d_calibration.xml", cv::FileStorage::WRITE);
	if(fs.isOpened()){
		fs << "Size" << imgsize;
	...
	for (int i = 0; i < normalimages.size(); i++){
		...
		cv::imwrite("left_image.jpg", undistortnormalImage[0]);
		cv::imwrite("right_image.jpg", undistortnormalImage[0]);
